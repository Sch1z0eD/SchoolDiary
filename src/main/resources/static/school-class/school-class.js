...
function detailFormatter1(index, row) {
    ...
    const deleteButton = `<button type ='button' data-toggle='modal'  data-target='#deleteModal' class='btn btn-warning mr-2'>удалить класс</button>`;

    const addClassTeacher = ` <button type='button' data-toggle='modal' data-target='#addTeacherModal' class='btn btn-warning m-2'>Добавить классного руководителя</button> <div class="modal fade" id="addTeacherModal" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="addTeacherModalLabel" aria-hidden="true"> <div class="modal-dialog"> <div class="modal-content"> <div class="modal-header"> <h5 class="modal-title" id="addTeacherModalLabel">Смена школьного классного руководителя</h5> <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span> </button> </div> <div class="modal-body"> <div id="addTeacherBody">${createAddTeacherModalBody(row.classId)} </div> </div> <div class="modal-footer"> <button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть</button> <button type="button" class="btn btn-primary" 
 onClick="addTeacherToClass(${row.classId})" data-dismiss="modal">Сохранить</button> </div> </div> </div> </div>`;

    const delteModal = '<div class="modal fade" id="deleteModal" tabIndex="-1" role="dialog"><div class="modal-dialog" role="document">' +
        '<div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="deleteModal">Вы уверены, что хотите удалить класс навсегда?</h5>' +
        '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>' +
        '</div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть</button> ' +
        '<button type="button" class="btn btn-primary" onClick="deleteSchoolClassById(' + row.classId + ')" data-dismiss="modal">Удалить класс</button></div></div></div></div>';


    const modifyClass = "<a type ='button' href='../studentProfile/" + row.classId + "' " +
        "class='btn btn-primary'>Edit</a>";

    return "<div class='student-table-details'><h5>Детали:</h5></br><div class='student-table-details-row'>" + html.join('') + deleteButton + " " +
