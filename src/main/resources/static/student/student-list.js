...
function detailFormatter(index, row) {
    ...
    const deleteButton = `<button type ='button' data-toggle='modal'  data-target='#deleteModal' class='btn btn-warning mr-2'>удалить студента</button>`;
    const goToProfileButton = "<a type ='button' href='../studentProfile/" + row.student_id + "' " +
        "class='btn btn-primary'>Профиль студента</a>";

    const modal = '<div class="modal fade" id="deleteModal" tabIndex="-1" role="dialog"><div class="modal-dialog" role="document">' +
        '<div class="modal-content"><div class="modal-header"><h5 class="modal-title" id="deleteModal">Вы уверены, что хотите удалить пользователя навсегда?</h5>' +
        ...
        '<button type="button" class="btn btn-primary" onClick="deleteStudentById(' + row.student_id + ')" data-dismiss="modal">Удалить пользователя</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>';
